...
export async function copyImageFile(absolutePath: string): Promise<void> {
	try {
		...
		console.warn(
			'Failed to copy image content, falling back to file path:',
			error.message,
		)
...
export async function copyFilesMacOS(absolutePaths: string[]): Promise<void> {
	...
	if (normalized.length === 1) {
		clipboard.writeBuffer(
			'public.file-url',
			Buffer.from(`file://${encodeURI(normalized[0])}`, 'utf8'),
		)
...
export async function copyFilesWindows(absolutePaths: string[]): Promise<void> {
	...
	if (writeFilesWithElectronWriter(normalized)) {
		console.log(
			'File(s) copied to clipboard (Windows Chromium filenames writer)',
		)
		return
	}
	...
	if (normalized.length === 1) {
		clipboard.writeBuffer(
			'FileNameW',
			Buffer.from(`${normalized[0]}\0`, 'utf16le'),
		)
	...
	if (normalized.length > 1) {
		...
		clipboard.writeBuffer(
			'FileNameW',
			Buffer.from(`${fallbackPath}\0`, 'utf16le'),
		)
		if (verifyClipboardWrite('FileNameW')) {
			console.warn(
				'Windows multi-file paste is unavailable, downgraded to the last file via FileNameW',
			)
			return
		}
	}
	...
	if (verifyClipboardWrite('CF_HDROP')) {
		console.warn(
			'Windows multi-file clipboard fell back to raw CF_HDROP, browser/Electron paste compatibility is not guaranteed',
		)
		return
	}
	...
	if (normalized.length === 1) {
		clipboard.writeBuffer(
			'FileName',
			Buffer.from(`${normalized[0]}\0`, 'latin1'),
		)
		...
		console.warn(
			'Windows multi-file paste fell back to plain text of the last file path',
		)
		return
	}
...
export async function copyFilesLinux(absolutePaths: string[]): Promise<void> {
	...
	clipboard.writeBuffer(
		'text/uri-list',
		Buffer.from(formatUriList(fileUris), 'utf8'),
	)
	...
	if (normalized.length > 1) {
		...
		if (verifyClipboardWrite('text/uri-list')) {
			console.warn(
				'Linux multi-file paste is unavailable, downgraded to the last file via text/uri-list',
			)
			return
		}
		...
		clipboard.writeText(fallbackPath)
		console.warn(
			'Linux multi-file paste fell back to plain text of the last file path',
		)
		return
	}
...
export async function copyFilesToClipboard(
	...
	try {
		...
		if (
			...
			IMAGE_EXTENSIONS.has(path.extname(absolutePaths[0]).toLowerCase())
		) {
		...
		console.log(
			`Successfully copied ${absolutePaths.length} file(s) to clipboard`,
		)
		return true
	} catch (error: any) {
