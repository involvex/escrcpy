...
export const windowIPCPlugin: Plugin<void, WindowIPCPluginOptions> = {
	...
	apply(mainApp: ElectronApp, options: WindowIPCPluginOptions = {}) {
		const {channelPrefix = 'window', handlers = {}} = options
		...
		if (isMaximized) {
			...
			ipcxMain.handle(
				channel,
				(event: IpcMainInvokeEvent, callback: (state: boolean) => void) => {
					const win = BrowserWindow.fromWebContents(event.sender)
					if (!win) {
						return false
					}
		...
		if (open) {
			...
			ipcxMain.handle(
				...
				(
					...
					payload: {id: string; options?: unknown} = {id: ''},
				) => {
		...
		if (closeById) {
			...
			ipcxMain.handle(
				...
				(
					_event: IpcMainInvokeEvent,
					payload: {id: string; instanceId?: string} = {id: ''},
				) => {
