...
export class SandboxManager {
	...
	configureSandbox(): SandboxConfigResult {
		...
		try {
			...
			const forceEnable =
				this._sanitizeEnvVar(this.process.env.FORCE_SANDBOX) === '1'
			const forceDisable =
				this._sanitizeEnvVar(this.process.env.FORCE_NO_SANDBOX) === '1'
			...
			if (forceDisable && forceEnable) {
				console.warn(
					'Both FORCE_NO_SANDBOX and FORCE_SANDBOX are set, prioritizing FORCE_NO_SANDBOX',
				)
			}
			...
			console.error(
				'Critical error during sandbox configuration:',
				error.message,
			)
