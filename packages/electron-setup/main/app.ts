...
export function createElectronApp(config: ElectronAppConfig = {}): ElectronApp {
	...
	let mainWindowResolver:
		((app?: ElectronApp) => Promise<BrowserWindow | undefined>) | undefined
		...
		use(plugin, options) {
			...
			const conflictingPlugin = pending.find(
				p =>
					...
					p.priority === normalized.priority,
			)
	...
	mainApp.on<{error: unknown; state?: {name?: string}}>(
		'plugin:error',
		(data: unknown) => {
			const payload = data as {error: unknown; state?: {name?: string}}
			console.error(
				`❌ Plugin error${payload.state?.name ? ` in "${payload.state.name}"` : ''}:`,
				payload.error || data,
			)
		},
	)
	...
	function flushPending() {
		...
		while (changed) {
			...
			if (iterations > maxIterations) {
				...
				mainApp.emit(
					'plugin:error',
					new Error(`Circular dependency detected: ${cyclePlugins.join(', ')}`),
				)
				break
			}
	...
	function installPlugin(item: NormalizedPlugin) {
		...
		const api =
			typeof apply === 'function' ? apply(mainApp, pluginOptions) : undefined
...
function resolveDispose<TApi>(
	item: NormalizedPlugin<TApi>,
	api: TApi,
	...
	if (
		...
		typeof api === 'object' &&
		'dispose' in api &&
		typeof (api as any).dispose === 'function'
	) {
	...
	if (
		...
		typeof api === 'object' &&
		'destroy' in api &&
		typeof (api as any).destroy === 'function'
	) {
