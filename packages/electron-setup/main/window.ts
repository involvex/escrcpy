 ...
 * ```
 */
export const useWindowContext = windowContext.use as <
	TPayload = unknown,
>() => WindowContext<TPayload>
...
export function createWindowManager<TPayload = unknown>(
	...
	function resolveOptions(
		...
		base.webPreferences = {
			...
			additionalArguments: base.webPreferences?.additionalArguments ??
				mainApp?.additionalArguments ?? [`--payload=${encodePayload(payload)}`],
		}
	...
	async function createWindow(
		...
		const context: WindowContext<TPayload> = {
			id: name,
			mainApp,
	...
	function resolveTarget(
		payload?: TPayload & {instanceId?: string; win?: BrowserWindow},
	...
	function bindWindowEvents(
		internalWin: TemplateBrowserWindow,
		context: WindowContext<TPayload>,
	) {
	...
	async function runHook(
		hookName: keyof WindowHooks<TPayload>,
		...args: any[]
	) {
