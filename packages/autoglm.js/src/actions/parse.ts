...
function getElements(node: Expression, index: number = 1) {
	...
	const _elements = (node.expressions[index] as AssignmentExpression)
		.right as ArrayExpression
	return _elements.elements.map(
		e => (e as unknown as Literal).value as number,
	) as [number, number]
}
...
function getActionType(node: Expression) {
	if (node.type === 'SequenceExpression') {
		return ((node.expressions[0] as AssignmentExpression).right as Literal)
			.value as string
	}
