 ...
 * Create a do action.
 */
export function createDoAction<T extends Omit<BaseAction, '_metadata'>>(
	action: T,
): T & {_metadata: 'do'} {
	return {
		...action,
...
export class ActionHandler {
	...
	async execute(
		...
		try {
			...
			switch (action.action) {
				...
				case 'Swipe':
					return await this.executeSwipeAction(
						action,
						screenWidth,
						screenHeight,
					)
				...
				case 'Long Press':
					return await this.executeLongPressAction(
						action,
						screenWidth,
						screenHeight,
					)
				case 'Double Tap':
					return await this.executeDoubleTapAction(
						action,
						screenWidth,
						screenHeight,
					)
	...
	private convertRelativeToAbsolute(
		...
		return [
			(element[0] / 1000) * screenWidth,
			(element[1] / 1000) * screenHeight,
		]
	}
	 ...
	 * Execute tap action.
	 */
	private async executeTapAction(
		action: TapAction,
		screenWidth: number,
		screenHeight: number,
	): Promise<ActionResult> {
		const [x, y] = action.element
		const [absoluteX, absoluteY] = this.convertRelativeToAbsolute(
			[x, y],
			screenWidth,
			screenHeight,
		)
	 ...
	 */
	private async executeSwipeAction(
		action: SwipeAction,
		screenWidth: number,
		screenHeight: number,
	): Promise<ActionResult> {
		const [startX, startY] = this.convertRelativeToAbsolute(
			action.start,
			screenWidth,
			screenHeight,
		)
		const [endX, endY] = this.convertRelativeToAbsolute(
			action.end,
			screenWidth,
			screenHeight,
		)
	 ...
	 */
	private async executeLongPressAction(
		action: LongPressAction,
		screenWidth: number,
		screenHeight: number,
	): Promise<ActionResult> {
		const [x, y] = this.convertRelativeToAbsolute(
			action.element,
			screenWidth,
			screenHeight,
		)
	 ...
	 */
	private async executeDoubleTapAction(
	 ...
	 */
	private async executeTakeOverAction(
		action: TakeOverAction,
	): Promise<ActionResult> {
		if (this.callbacks.takeoverCallback) {
			await this.callbacks.takeoverCallback(
				action.reason || 'Manual takeover requested',
			)
			return {
				success: true,
				should_finish: false,
