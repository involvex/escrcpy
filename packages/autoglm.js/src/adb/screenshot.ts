...
export async function getScreenshot(
	...
	try {
		...
		const dimensionResult = await runAdbCommand(deviceId, [
			'shell',
			'wm',
			'size',
		])
		...
		// Parse dimensions
		const dimensionMatch = dimensionResult.stdout.match(
			/Physical size: (\d+)x(\d+)/,
		)
		if (!dimensionMatch) {
		...
		const screenshotResult = await runAdbCommand(
			deviceId,
			['shell', 'screencap', '-p', '/sdcard/tmp_screenshot.png'],
			timeout * 1000,
		)
		...
		await runAdbCommand(
			deviceId,
			['pull', '/sdcard/tmp_screenshot.png', tempPath],
			timeout * 5000,
		)
