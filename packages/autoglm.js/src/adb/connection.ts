...
export class ADBConnection {
	...
	async enableTCPIP(port: number = 5555, deviceId?: string) {
		try {
			const args = deviceId
				? ['-s', deviceId, 'tcpip', port.toString()]
				: ['tcpip', port.toString()]
			const result = await exec(this.adb, args)
			return {success: true, message: result.stdout.trim()}
		} catch (error) {
	...
	async getDeviceIp(deviceId?: string) {
		try {
			const args = deviceId
				? ['-s', deviceId, 'shell', 'ip', 'addr', 'show', 'wlan0']
	...
	async listDevices(): Promise<DeviceInfo[]> {
		...
		const devices = await Promise.all(
			deviceLines.map(async line => {
				const parts = line.split(/\s+/)
				const deviceId = parts[0]
				const status = parts[1] as 'device' | 'unauthorized' | 'offline'

				// Determine connection type
				const connectionType = deviceId.includes(':')
				...
				if (status === 'device') {
					try {
						const [brand, manufacturer, device, androidVersion, apiLevel] =
							await Promise.all([
	 ...
	 */
	private async getDeviceProp(
		deviceId: string,
		prop: string,
	): Promise<string | null> {
		try {
			const result = await exec(this.adb, [
				'-s',
				deviceId,
				'shell',
				'getprop',
