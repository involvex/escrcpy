...
function cloneValue(
	...
	if (typeof value === 'function') {
		const label =
			(value as Function).name || String(segments[segments.length - 1] ?? 'fn')
	...
	if (Array.isArray(value)) {
		...
		try {
			...
			throw createSerializeError('Failed to serialize array', error, {
				segments,
	...
	if (isPlainObject(value)) {
		...
		try {
			...
			throw createSerializeError('Failed to serialize object', error, {
				segments,
...
export function serializeArgs(
	...
	try {
		const sanitizedArgs = cloneValue(
			args,
			[],
			descriptors,
			baseChannel,
		...
		const result = Array.isArray(sanitizedArgs)
			? sanitizedArgs
			: [sanitizedArgs]
		...
		throw createSerializeError('Serialization failed', error, {
