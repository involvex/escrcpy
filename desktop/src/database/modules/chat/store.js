...
class ChatMessageStore extends BaseStore {
  ...
  async getSessionMessagesPaginated(sessionId, params = {}) {
    try {
      const { page = 1, pageSize = 50, beforeTimestamp } = params
      ...
      console.error(
        '[ChatMessageStore] getSessionMessagesPaginated error:',
        error,
      )
      return {
        success: false,
  ...
  async getMessagesByTimeRange(sessionId, startTime, endTime) {
    try {
      const records = await this.table
        ...
        .between([sessionId, startTime], [sessionId, endTime], true, true)
        .toArray()
  ...
  async getSessionStats(sessionId) {
    try {
      ...
      const stats = {
        ...
        assistantCount: messages.filter(m => m.role === MessageRole.ASSISTANT)
          .length,
