...
async function executeWithRetryForDevice(device, command, taskConfig) {
  ...
  try {
    ...
    if (error.message === 'EXECUTION_TIMEOUT') {
      const errorMsg = window.t('copilot.batch.error.timeout', {
...
async function executeCopilotCommandForDevice(device, command) {
  ...
  if (!config.apiKey) {
    throw copilotClient.formatError(
      'CONFIG_MISSING',
      window.t('copilot.batch.error.missingApiKey'),
    )
  }
...
async function handleSubmit(command) {
  ...
  const selectedDevices = deviceSelectionHelper.filter(
    props.devices,
    'onlineAndUnique',
  )
