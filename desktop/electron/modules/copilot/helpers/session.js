...
export class SessionManager {
  ...
  constructor(options = {}) {
    ...
    this.idleTimeout
      = options.idleTimeout || TIME_CONSTANTS.DEFAULT_IDLE_TIMEOUT
  ...
  getOrCreateSession(deviceId, config) {
    ...
    if (!deviceId || typeof deviceId !== 'string') {
      throw new Error(
        `${LOG_CONFIG.PREFIX} Device ID must be a non-empty string`,
      )
    }
    if (!config || typeof config !== 'object') {
      throw new Error(
        `${LOG_CONFIG.PREFIX} Session config must be a valid object`,
      )
    }
  ...
  _createNewSession(deviceId, config) {
    ...
    console.log(
      `${LOG_CONFIG.PREFIX} Created new session: ${sessionId} (Device: ${deviceId})`,
    )
    return session
  }
  ...
  destroySession(deviceId) {
    ...
    if (!session) {
      console.warn(
        `${LOG_CONFIG.PREFIX} No valid session found for device ${deviceId}, no need to destroy`,
      )
      return
    }
    ...
    try {
      ...
      console.log(
        `${LOG_CONFIG.PREFIX} Destroyed session: ${session.id} (Device: ${deviceId})`,
      )
    }
    catch (error) {
      console.error(
        `${LOG_CONFIG.PREFIX} Failed to destroy session for device ${deviceId}:`,
        error.message,
      )
    }
  }
  ...
  destroyAll() {
    ...
    console.log(
      `${LOG_CONFIG.PREFIX} Starting to destroy all sessions (total: ${sessionCount})`,
    )
    ...
    try {
      ...
      console.error(
        `${LOG_CONFIG.PREFIX} Failed to destroy all sessions:`,
        error.message,
      )
    }
  }
  ...
  async executeTask(deviceId, task, config, callbacks = {}) {
    ...
    if (!task || typeof task !== 'string') {
      throw new Error(
        `${LOG_CONFIG.PREFIX} Task description must be a non-empty string`,
      )
    }
    ...
    try {
      ...
      console.error(
        `${LOG_CONFIG.PREFIX} Failed to execute task on device ${deviceId}:`,
        error.message,
      )
  ...
  async executeTaskList(task, deviceIds, sessionConfig, options = {}) {
    ...
    if (!Array.isArray(deviceIds) || deviceIds.length === 0) {
      throw new Error(
        `${LOG_CONFIG.PREFIX} Device ID list must be a non-empty array`,
      )
    }
    ...
    const createDeviceTaskExecutor = deviceId =>
      limit(async () => {
  ...
  stopTask(deviceId, reason = 'User abort') {
    ...
    if (!session) {
      console.warn(
        `${LOG_CONFIG.PREFIX} No session found for device ${deviceId}, cannot stop task`,
      )
      return
    }
    ...
    try {
      if (typeof session.agent.abort === 'function') {
        ...
        console.log(
          `${LOG_CONFIG.PREFIX} Stopped task on device ${deviceId}: ${reason}`,
        )
        ...
        console.warn(
          `${LOG_CONFIG.PREFIX} Agent for device ${deviceId} does not support abort operation`,
        )
      }
    }
    catch (error) {
      console.error(
        `${LOG_CONFIG.PREFIX} Failed to stop task on device ${deviceId}:`,
        error.message,
      )
    }
  }
  ...
  _checkAndRecycleIdleSessions() {
    ...
    if (idleDeviceIds.length > 0) {
      console.log(
        `${LOG_CONFIG.PREFIX} Recycled ${idleDeviceIds.length} idle sessions`,
      )
    }
  }
  ...
  setIdleTimeout(timeout) {
    if (typeof timeout !== 'number' || timeout <= 0) {
      console.warn(
        `${LOG_CONFIG.PREFIX} Invalid idle timeout: ${timeout} (must be a positive integer)`,
      )
      return
    }
    ...
    console.log(
      `${LOG_CONFIG.PREFIX} Idle timeout updated to: ${timeout / 1000}s`,
    )
  }
  ...
  _getConcurrencyLimit(customLimit) {
    return Number(
      customLimit ?? electronStore.get('common.concurrencyLimit') ?? 5,
    )
  }
  ...
  _attachAgentEventHandlers(session) {
    ...
    const forwardAgentEvent = (event, payload) => {
      ...
      if (
        ...
        && ['finish'].includes(payload?.message?._metadata)
      ) {
        return false
      }
  ...
  _categorizeTaskResults(results) {
    ...
    if (failureList.length > 0) {
      console.warn(
        `${LOG_CONFIG.PREFIX} Batch task execution failed count: ${failureList.length}`,
        failureList,
      )
    }
  ...
  _executeCleanupFunctions(cleanupFunctions) {
    cleanupFunctions.forEach((cleanup) => {
      try {
        ...
        console.warn(
          `${LOG_CONFIG.PREFIX} Failed to execute cleanup function:`,
          e.message,
        )
      }
    })
