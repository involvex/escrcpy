  ...
  >
    ...
    <div class="p-2">
      <template v-for="item of reverseMessages" :key="item.id">
        <ChatItem
          v-bind="{ item, messages, deleteMessage, handleSubmit }"
        ></ChatItem>
      </template>
    </div>
    ...
    <template #footer>
      ...
      <div
        v-if="scrollTop < 0"
        class="absolute left-center -top-28"
...
import {
  adaptMessagesForTDesign,
  createTemporaryAssistantMessage,
} from '$copilot/utils/messageAdapter.js'
import {
  MessageRoleEnum,
  MessageStatusEnum,
  TaskStatusEnum,
...
const { messages, loading, addMessage, deleteMessage, clearAll }
...
async function handleSubmit(text) {
  ...
  try {
    ...
    if (
      [MessageStatusEnum.PENDING, MessageStatusEnum.RUNNING].includes(
        temporaryMessage.value.status,
      )
    ) {
      throw new Error(
