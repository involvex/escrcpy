...
export class PreflightChecker {
  ...
  async checkADBKeyboard(deviceId) {
    ...
    try {
      const isInstalled
        = await window.$preload.adb.isInstalledAdbKeyboard(deviceId)
      ...
      return {
        ...
        message: window.t('copilot.check.adb.checkError', {
          error: error.message,
        }),
        actionType: 'none',
    ...
    try {
      ...
      const result = {
        ...
        message: apiStatus.success
          ...
          : window.t('copilot.check.api.failure', {
              message:
              apiStatus.message || window.t('copilot.check.api.unknownError'),
            }),
      ...
      return {
        name: window.t('copilot.check.api.name'),
        status: CheckStatus.FAIL,
        message: window.t('copilot.check.api.checkError', {
          error: error.message,
        }),
        actionType: 'configure',
        details: { error: error.message },
